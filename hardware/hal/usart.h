...
static inline void usart_use_tx_fifo(usart_dev *dev, uint8 enable) {
	if (enable > 0)
	{
		dev->usetxrb=1;
	} else {
...
static inline void usart_use_timeout(usart_dev *dev, uint8 enable) {
	if (enable > 0)
	{
		dev->use_timeout=1;
	} else {
...
static inline void usart_set_timeout(usart_dev *dev, uint32 timeout) {
	dev->tx_timeout = timeout;
}

static inline uint32 usart_txfifo_nbytes(usart_dev *dev) {
	return rb_full_count(dev->txrb);
}
static inline uint32 usart_txfifo_freebytes(usart_dev *dev) {
