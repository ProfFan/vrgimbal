...
#ifdef AP_InertialSensor_MPU6000_I2C_DEBUG_ENABLE
#pragma message "*** AP_InertialSensor_MPU6000_I2C Debug Enabled ***"
...
/* Inertial Sensors *********************************************/
#define MPU6000_INT_PIN			99  //vedi i file BOARD

// MPU6000 accelerometer scaling
#define MPU6000_ACCEL_SCALE_1G    (GRAVITY / 4096.0)


#define MPU6000_ADDR_LO 0x68
#define MPU6000_ADDR_HI 0x69
#define MPU6000_ADDR_DEFAULT MPU6000_ADDR_HI
...
HardwareI2C *AP_InertialSensor_MPU6000_I2C::_I2Cx;
uint8_t AP_InertialSensor_MPU6000_I2C::_address = MPU6000_ADDR_DEFAULT; 
uint8 AP_InertialSensor_MPU6000_I2C::_interrupt_pin = -1;
...
const float AP_InertialSensor_MPU6000_I2C::_gyro_scale = (0.017453292519 / 32.8);

/* pch: I believe the accel and gyro indicies are correct
 *      but somone else should please confirm.
 */
const uint8_t AP_InertialSensor_MPU6000_I2C::_gyro_data_index[3]  = { 5, 4, 6 };
...
int16_t AP_InertialSensor_MPU6000_I2C::_mpu6000_product_id = AP_PRODUCT_ID_NONE;
...
bool AP_InertialSensor_MPU6000_I2C::_dmp_initialised = false;
uint8_t AP_InertialSensor_MPU6000_I2C::_fifoCountH;			// high byte of number of elements in fifo buffer
uint8_t AP_InertialSensor_MPU6000_I2C::_fifoCountL;			// low byte of number of elements in fifo buffer
Quaternion AP_InertialSensor_MPU6000_I2C::quaternion;			// holds the 4 quaternions representing attitude taken directly from the DMP
...
float AP_InertialSensor_MPU6000_I2C::temperature() {
    return _temp;
}

/*================ HARDWARE FUNCTIONS ==================== */

int16_t AP_InertialSensor_MPU6000_I2C::i2c_transfer_16(uint8_t address)
{
	uint8_t byte_H, byte_L;
	uint8_t byte[2];
	_I2Cx->read(_address, address, 2, byte);
	byte_H = byte[0];
	byte_L = byte[1];
	//_I2Cx->read(_address, (uint8_t)address, 	   1, &byte_H);
	//_I2Cx->read(_address, (uint8_t) (address + 1) , 1, &byte_L);
...
/*
  ...
  and add it to _sum[]
 */
void AP_InertialSensor_MPU6000_I2C::read(uint32_t )
{
...
void AP_InertialSensor_MPU6000_I2C::read()
{
	AP_InertialSensor_MPU6000_I2C::read((uint32_t)micros());
}

uint8_t AP_InertialSensor_MPU6000_I2C::register_read( uint8_t reg )
{
  uint8_t return_value;
  uint8_t addr = reg | 0x80; // Set most significant bit

//  if (serPort)
//	  serPort->printf("r%02x", reg);
  uint8_t ret = _I2Cx->read(_address, addr, 1, &return_value);
//  if (serPort)
//	  serPort->printf("%02x%02x.\r\n", return_value, ret);
  return return_value;
}

void AP_InertialSensor_MPU6000_I2C::register_write(uint8_t reg, uint8_t val)
{
...
//	if (serPort)
//		serPort->printf("w%02x%02x", reg, val);
	uint8_t ret = _I2Cx->write(_address, reg, val);
 //   if (serPort)
 //   	serPort->printf("%02x.\r\n", ret);
}


void AP_InertialSensor_MPU6000_I2C::register_write_bits(uint8_t reg, uint8_t mask, uint8_t val)
{
	uint8_t origval = register_read(reg);
	uint8_t newval = 0;
	if (val == 0)
		newval = origval & (~mask);
	else
		newval = origval | (mask);
	register_write(reg, newval);
}


// MPU6000 new data interrupt on INT6
void AP_InertialSensor_MPU6000_I2C::data_interrupt(void)
{
...
//#ifndef BOARD_INERTIALSENSOR_INT
//#pragma message "*** MPU6000 interrupt pin not defined ***"
//#endif
	pinMode(_interrupt_pin, INPUT_PULLUP);

	attachInterrupt(_interrupt_pin,data_interrupt,RISING);
    delay(2);
...
void AP_InertialSensor_MPU6000_I2C::push_gyro_offsets_to_dmp()
{
...
void AP_InertialSensor_MPU6000_I2C::set_dmp_gyro_offsets(int16_t offsetX, int16_t offsetY, int16_t offsetZ)
{
    int16_t aux_int;
...
void AP_InertialSensor_MPU6000_I2C::push_accel_offsets_to_dmp()
{
    Vector3f accel_offset = _accel_offset.get();
    Vector3f accel_scale = _accel_scale.get();
...
void AP_InertialSensor_MPU6000_I2C::set_dmp_accel_offsets(int16_t offsetX, int16_t offsetY, int16_t offsetZ)
{
    int aux_int;
    uint8_t regs[2];
...
void AP_InertialSensor_MPU6000_I2C::dmp_register_write(uint8_t bank, uint8_t address, uint8_t num_bytes, uint8_t data[])
{
...
//	  digitalWrite(_cs_pin, HIGH);
    for (uint8_t i=0; i<num_bytes; i++) {
    	_I2Cx->write((uint8_t)_address,(uint8_t)( address + i), (uint8_t) (data[i]));
    }
}

// MPU6000 DMP initialization
// this should be called after hardware_init if you wish to enable the dmp
void AP_InertialSensor_MPU6000_I2C::dmp_init()
{
...
void AP_InertialSensor_MPU6000_I2C::dmp_reset()
{
...
bool AP_InertialSensor_MPU6000_I2C::FIFO_ready()
{
...
void AP_InertialSensor_MPU6000_I2C::FIFO_reset()
{
    uint8_t temp;
...
void AP_InertialSensor_MPU6000_I2C::FIFO_getPacket()
{
    uint8_t i;
    int16_t q_long[4];
...
//	digitalWrite(_cs_pin, HIGH);				// disable device
    ...
    _I2Cx->read(_address, addr, _fifoCountL, received_packet);
...
void AP_InertialSensor_MPU6000_I2C::dmp_set_gyro_calibration()
{
    uint8_t regs[4];
    regs[0]=0x4C;
    regs[1]=0xCD;
...
void AP_InertialSensor_MPU6000_I2C::dmp_set_accel_calibration()
{
    uint8_t regs[6];
    regs[0]=0x00;
    regs[1]=0x00;
...
void AP_InertialSensor_MPU6000_I2C::dmp_apply_endian_accel()
{
    uint8_t regs[4];
    regs[0]=0x00;
    regs[1]=0x00;
...
void AP_InertialSensor_MPU6000_I2C::dmp_set_mpu_sensors()
{
    uint8_t regs[6];
    regs[0]=0x0C;
    regs[1]=0xC9;
...
void AP_InertialSensor_MPU6000_I2C::dmp_set_bias_from_no_motion()
{
    uint8_t regs[4];
    regs[0]=0x0D;
    regs[1]=0x35;
...
void AP_InertialSensor_MPU6000_I2C::dmp_set_bias_none()
{
    uint8_t regs[4];
    regs[0]=0x98;
    regs[1]=0x98;
...
// dmp_set_fifo_interrupt
void AP_InertialSensor_MPU6000_I2C::dmp_set_fifo_interrupt()
{
    uint8_t regs[1];
    regs[0]=0xFE;
...
void AP_InertialSensor_MPU6000_I2C::dmp_send_quaternion()
{
    uint8_t regs[5];
    regs[0]=0xF1;
    regs[1]=0x20;
...
void AP_InertialSensor_MPU6000_I2C::dmp_send_gyro()
{
    uint8_t regs[4];
    regs[0]=0xF1;
    regs[1]=0x28;
...
void AP_InertialSensor_MPU6000_I2C::dmp_send_accel()
{
    uint8_t regs[54];
    regs[0]=0xF1;
    regs[1]=0x28;
...
void AP_InertialSensor_MPU6000_I2C::dmp_set_fifo_rate(uint8_t rate)
{
    uint8_t regs[2];
    regs[0]=0x00;
    regs[1]=rate;
...
void AP_InertialSensor_MPU6000_I2C::dmp_set_sensor_fusion_accel_gain(uint8_t gain)
{
	//inv_key_0_96
...
//	digitalWrite(_cs_pin, HIGH);
	...
	byte buff[4] = {0x00, gain, 0x00, 0x00};
	for (int i = 0; i < 4; i++)
	{
		_I2Cx->write((uint8_t) _address, (uint8_t) (0x60 + i), (uint8_t) (buff[i]));
	}

}

// Load initial memory values into DMP memory banks
void AP_InertialSensor_MPU6000_I2C::dmp_load_mem()
{
	/*
	for(int i = 0; i < 7; i++) {
