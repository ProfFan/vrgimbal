...
uint8 AP_InertialSensor_MPU6000::_interrupt_pin = -1;
uint8 AP_InertialSensor_MPU6000::_cs_active_high = false;
...
//AP_InertialSensor_MPU6000::AP_InertialSensor_MPU6000( uint8_t cs_pin, HardwareSPI *spi_dev, int interrupt_pin, FastSerial *ser_port)
AP_InertialSensor_MPU6000::AP_InertialSensor_MPU6000(HardwareSPI *spi_dev, uint8_t cs_pin, bool bCSActiveHigh, int interrupt_pin, FastSerial *ser_port)
{
  serPort = ser_port;
  _cs_pin = cs_pin; /* can't use initializer list,  is static */
  _cs_active_high = bCSActiveHigh;
  _temp = 0;
  _initialised = false;
...
void AP_InertialSensor_MPU6000::spi_enable(bool enable)
{
	if (!_cs_active_high)
		enable = !enable;
	digitalWrite(_cs_pin, (enable ? HIGH: LOW));
}
