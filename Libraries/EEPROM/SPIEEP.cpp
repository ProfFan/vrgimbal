...
SPIEEP::SPIEEP(HardwareSPI * spi_dev, int cspin, bool csActiveLow ,int addrwidth, int pagesize, int totalsize) {
	_SPIx = spi_dev;
	this->init(spi_dev, cspin, csActiveLow ,addrwidth, pagesize, totalsize);
}
...
void SPIEEP::init(HardwareSPI * spi_dev, int cspin, bool csActiveLow ,int addrwidth, int pagesize, int totalsize) {
	_SPIx = spi_dev;
	_cspin = (uint8_t)cspin;
	_csActiveLow = csActiveLow;
...
void SPIEEP::_csEnable(bool enable)
{
	uint8_t val = HIGH;
	if (enable)
	{
	...
	//TODO verifica necessità DELAY
//	if (enable)
//		delay(5);
	digitalWrite(_cspin, val);
//	if (!enable)
//		delay(5);
}
...
void SPIEEP::begin(int cspin) {
  ...
  if ((SPCR >> SPE) & 0x1) {
    _cspin = cspin;
    pinMode(_cspin, OUTPUT);
    _csEnable(false);  // CS pin is high when idle.
  } else
...
void SPIEEP::begin_spi(int cspin) {
  ...
  _SPIx->begin();
  _SPIx->setBitOrder(MSBFIRST);
  _SPIx->setDataMode(SPI_MODE0);
  _SPIx->setClockDivider(SPI_CLOCK_DIV2);
...
byte SPIEEP::readstatus() {
  ...
  _csEnable(true);
    _SPIx->transfer(SPIEEP_RDSR);
    ret = _SPIx->transfer(0x0);
  _csEnable(false);
...
void SPIEEP::_write_address(uint32_t p) {
  ...
  for (i=_addrwidth-1; i>=0; i--)
    _SPIx->transfer( (byte)(p >> (8*i)) & 0xFF );
}
...
byte SPIEEP::read(uint32_t p) {
  ...
  _csEnable(true);
    if (_is_25LC040)
      _SPIx->transfer(_compose_instruction_addrbit(p, SPIEEP_READ));
    else
      _SPIx->transfer(SPIEEP_READ);
    _write_address(p);
    ret = _SPIx->transfer(0x0);
  _csEnable(false);
...
boolean SPIEEP::write(uint32_t p, byte b) {
  if (!verify_enabled()) return false;
//TODO verificare protocollo comandi abilitazione scrittura e lettura registro
  wren();
  if (!is_wren())
    return false;  // Couldn't enable WREN for some reason?
  _csEnable(true);
    if (_is_25LC040)
      _SPIx->transfer(_compose_instruction_addrbit(p, SPIEEP_WRITE));
    else
      _SPIx->transfer(SPIEEP_WRITE);
    _write_address(p);
    _SPIx->transfer(b);
  _csEnable(false);
...
boolean SPIEEP::readn(uint32_t p, byte *buf, uint16_t len) {
  ...
  do {
    ...
    if (e > pg_i_e)
      ...
      _csEnable(true);
      if (_is_25LC040)
        _SPIx->transfer(_compose_instruction_addrbit(idx, SPIEEP_READ));
      else
        _SPIx->transfer(SPIEEP_READ);
        _write_address(idx);
        for (i=0; i<rdlen; i++)
          buf[idx-p+i] = _SPIx->transfer(0x0);
      _csEnable(false);
...
boolean SPIEEP::writen(uint32_t p, byte *buf, uint16_t len) {
  ...
  do {
    ...
    if (e > pg_i_e)
      ...
      _csEnable(true);
      if (_is_25LC040)
        _SPIx->transfer(_compose_instruction_addrbit(idx, SPIEEP_WRITE));
      else
        _SPIx->transfer(SPIEEP_WRITE);
        _write_address(idx);
        for (i=0; i<wrlen; i++)
          _SPIx->transfer(buf[idx-p+i]);
      _csEnable(false);
