...
// constructor
AP_PerfMon::AP_PerfMon(uint8_t funcNum, FastSerial* ser_port) :
	_funcNum(funcNum),
	_time_this_iteration(0)
{
	_serExt = ser_port;
...
void AP_PerfMon::DisplayResults()
{
    ...
    blocking_writes = _serExt->get_blocking_writes();
    _serExt->set_blocking_writes(true);
    _serExt->printf("\nblocking was:%d\n",(int)blocking_writes);

    // print table of results
    _serExt->printf("\nPerfMon elapsed:%lu(ms)\n",(unsigned long)totalTime/1000);
    _serExt->printf("Fn:\t\t    cpu\t  tot(ms)\t  avg(ms)\t  max(ms)\t  #calls\t    Hz\n");
    for( i=0; i<nextFuncNum; i++ ) {
        j=order[i];
        sumOfTime += time[j];
        ...
        _serExt->printf("%-10s\t  %4.2f\t  %lu\t  %4.3f\t  %4.3f\t  %lu\t  %4.1f\n",
            functionNames[j],
            pct,
